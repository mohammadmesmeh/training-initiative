...
export default function AdminPage() {
  ...
  const [winner, setWinner] = useState(null); // { id, name }
  ...
  const handleDraw = () => {
    ...
    setTimeout(() => {
      ...
      setWinner({ id: random.id, name: random.name });
      setIsDrawing(false);
    }, 600);
  };
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
                  ...
                  <div className="flex items-center justify-center gap-5">
                    <div className="animate-pulse-ring w-20 h-20 rounded-2xl bg-[#00BFA6] flex flex-col items-center justify-center shrink-0 shadow-lg shadow-[#00BFA6]/40">
                      ...
                      <span className="text-3xl font-extrabold text-white leading-none">
                        {names.findIndex(n => n.id === winner.id) + 1}
                      </span>
                    </div>
                    <div className="text-left">
                      <p className="text-3xl font-extrabold leading-tight">{winner.name}</p>
                      <p className="text-slate-400 text-sm mt-1">
                        Entry #{names.findIndex(n => n.id === winner.id) + 1} of {names.length}
                      </p>
                    </div>
                  </div>
                </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <button
            ...
            className={`flex-1 cursor-pointer flex items-center justify-center gap-2 py-4 rounded-xl font-semibold text-sm transition-all duration-150 shadow-sm ${
          ...
          {names.length > 0 && (
            <button
              onClick={handleClear}
              className="sm:w-auto cursor-pointer px-5 py-4 rounded-xl border border-slate-200 text-slate-500 hover:border-red-200 hover:text-red-500 hover:bg-red-50 text-sm font-medium transition-all duration-150 flex items-center justify-center gap-2"
            >
                  ...
                  >
                    ...
                    <div className="ml-auto flex items-center gap-3">
                      <button
                        ...
                        className="rounded-full cursor-pointer border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-500 transition-colors duration-150 hover:border-red-200 hover:text-red-500 hover:bg-red-50"
                      >
                        Delete
                      </button>
                      {isWinner && (
