...
export default function InputPage() {
  ...
  const handleSubmit = (e) => {
    ...
    setError('');
    const success = addName(trimmed);
    if (success) {
      setConfirmation({ name: trimmed, id: Date.now() });
      setInput('');
      inputRef.current?.focus();
    }
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  autoComplete="off"
                  className={`w-full px-4 py-3 rounded-xl border text-sm text-[#0F1F3D] placeholder-slate-300 outline-none transition-all duration-200 focus:ring-2 focus:ring-[#00BFA6]/40 focus:border-[#00BFA6] ${
              ...
              <button
                type="submit"
                className="w-full bg-[#00BFA6] hover:bg-[#009E89] active:scale-[0.98] text-white font-semibold py-3 rounded-xl text-sm transition-all duration-150 shadow-sm shadow-[#00BFA6]/30"
              >
                Register Name
              </button>
            </form>
